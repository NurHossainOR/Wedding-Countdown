...
export async function POST() {
  try {
    ...
    const existingCount = await prisma.dailyMessage.count()
    
    if (existingCount > 0) {
      return NextResponse.json({
        message: 'Messages already exist. Delete existing messages first if you want to reseed.',
        count: existingCount,
      })
    }

    // Create all messages
    const messages = await prisma.dailyMessage.createMany({
      data: islamicMessages.map((text) => ({ text })),
    })

    return NextResponse.json({
      success: true,
      message: `Created ${messages.count} daily messages`,
    })
  } catch (error) {
